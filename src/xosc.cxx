...
{
    if (m_expression) { attr = m_expression.value(); return;  }  
    if (m_parameter) { attr = m_parameter.value(); return;  }  
...
ScenarioDefinition::ScenarioDefinition(pugi::xml_node node)
{
    ...
    if (node.child("RoadNetwork")) { 
        m_RoadNetwork = std::make_shared<RoadNetwork>(node.child("RoadNetwork")); 
    }  
